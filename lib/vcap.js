...
var VcapClient = module.exports = function (info, callback) {
    ...
    var login = function (callback) {
        ...
        makeRequest(url.toUaa('https://' + info.host), null, 'POST',
            { password: info.password }, function (err, result) {
            if (err) {
                return callback(err);
            }
    ...
    var InnerCollection = function (id, collection, parent, schema) {
        this.get = function (innerCollection, callback) {
            authedRequest(getPath(collection, id, innerCollection), null, 'GET',
                ...
                callback(err, result.resources || result);
            });
        };
    };
    ...
    var Collection = function (collection, schema) {
        ...
        this.delete = function (id, callback) {
            authedRequest(getPath(collection, id), null, 'DELETE', true, callback);
        };
    };
    ...
    var setAppState = function (id, state, callback) {
        self.apps.get(id, function (err, app) {
            if (err) {
                return callback(err);
            }

            if (app.entity.state === state) {
                return callback();
            }

            app.entity.state = state;
            self.apps.update(id, app.entity, callback);
        });
    };
